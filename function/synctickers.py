...
def lambda_handler(event, context):
    ...
    candles = myexch.fetchOHLCV(market_symbol, timeframe, limit=cl.limit_1m)
    ...
    timeframe='1h'
    key = exchange+'_'+market_symbol+'_'+timeframe
    candles = myexch.fetchOHLCV(market_symbol, timeframe, limit=cl.limit_1h)
    save_candles(exchange, market_symbol, timeframe, nowdt, candles)

    # Get day timeframe OHLCV candles data, grouped per month on a table entry
    timeframe='1d'
    key = exchange+'_'+market_symbol+'_'+timeframe
    candles = myexch.fetchOHLCV(market_symbol, timeframe, limit=cl.limit_1d)
    save_candles(exchange, market_symbol, timeframe, nowdt, candles)
